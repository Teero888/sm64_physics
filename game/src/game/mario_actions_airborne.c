...
s32 lava_boost_on_wall(struct MarioState *m) {
    N64_STACK_FRAME(lava_boost_on_wall);
...
u32 common_air_action_step(struct MarioState *m, u32 landAction, s32 animation, u32 stepArg) {
    N64_STACK_FRAME(common_air_action_step);
    u32 stepResult;
...
s32 act_double_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_double_jump);
...
s32 act_triple_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_triple_jump);
...
s32 act_hold_freefall(struct MarioState *m) {
    N64_STACK_FRAME(act_hold_freefall);
    s32 animation;
    if (m->actionArg == 0) {
...
s32 act_wall_kick_air(struct MarioState *m) {
    N64_STACK_FRAME(act_wall_kick_air);
    if (m->input & INPUT_B_PRESSED) {
        return set_mario_action(m, ACT_DIVE, 0);
    }
...
s32 act_riding_shell_air(struct MarioState *m) {
    N64_STACK_FRAME(act_riding_shell_air);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
...
s32 act_water_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_water_jump);
...
s32 act_hold_water_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_hold_water_jump);
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) {
        return drop_and_set_mario_action(m, ACT_FREEFALL, 0);
    }
...
s32 act_steep_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_steep_jump);
    if (m->input & INPUT_B_PRESSED) {
        return set_mario_action(m, ACT_DIVE, 0);
    }
...
s32 act_crazy_box_bounce(struct MarioState *m) {
    N64_STACK_FRAME(act_crazy_box_bounce);
    f32 minSpeed;
...
u32 common_air_knockback_step(struct MarioState *m, u32 landAction, u32 hardFallAction, s32 animation,
                              f32 speed) {
    N64_STACK_FRAME(common_air_knockback_step);
    u32 stepResult;
...
s32 act_backward_air_kb(struct MarioState *m) {
    N64_STACK_FRAME(act_backward_air_kb);
...
s32 act_forward_air_kb(struct MarioState *m) {
    N64_STACK_FRAME(act_forward_air_kb);
    if (check_wall_kick(m)) {
        return TRUE;
    }
...
s32 act_hard_backward_air_kb(struct MarioState *m) {
    N64_STACK_FRAME(act_hard_backward_air_kb);
#ifndef VERSION_JP
    play_knockback_sound(m);
#else
    play_sound_if_no_flag(m, SOUND_MARIO_UH, MARIO_MARIO_SOUND_PLAYED);
...
s32 act_hard_forward_air_kb(struct MarioState *m) {
    N64_STACK_FRAME(act_hard_forward_air_kb);
#ifndef VERSION_JP
    play_knockback_sound(m);
#else
    play_sound_if_no_flag(m, SOUND_MARIO_UH, MARIO_MARIO_SOUND_PLAYED);
...
s32 act_thrown_backward(struct MarioState *m) {
    N64_STACK_FRAME(act_thrown_backward);
    u32 landAction;
    if (m->actionArg != 0) {
...
s32 act_thrown_forward(struct MarioState *m) {
    N64_STACK_FRAME(act_thrown_forward);
    s16 pitch;
...
s32 act_forward_rollout(struct MarioState *m) {
    N64_STACK_FRAME(act_forward_rollout);
    if (m->actionState == 0) {
        m->vel[1] = 30.0f;
        m->actionState = 1;
    }
...
s32 act_backward_rollout(struct MarioState *m) {
    N64_STACK_FRAME(act_backward_rollout);
    if (m->actionState == 0) {
        m->vel[1] = 30.0f;
        m->actionState = 1;
    }
...
s32 act_butt_slide_air(struct MarioState *m) {
    N64_STACK_FRAME(act_butt_slide_air);
...
s32 act_hold_butt_slide_air(struct MarioState *m) {
    N64_STACK_FRAME(act_hold_butt_slide_air);
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) {
...
s32 act_lava_boost(struct MarioState *m) {
    N64_STACK_FRAME(act_lava_boost);
#if ENABLE_RUMBLE
...
s32 act_slide_kick(struct MarioState *m) {
    N64_STACK_FRAME(act_slide_kick);
...
s32 act_shot_from_cannon(struct MarioState *m) {
    N64_STACK_FRAME(act_shot_from_cannon);
...
s32 act_flying_triple_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_flying_triple_jump);
#ifndef VERSION_JP
...
s32 act_top_of_pole_jump(struct MarioState *m) {
    N64_STACK_FRAME(act_top_of_pole_jump);
    play_mario_jump_sound(m);
...
s32 check_common_airborne_cancels(struct MarioState *m) {
    N64_STACK_FRAME(check_common_airborne_cancels);
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    N64_STACK_FRAME(mario_execute_airborne_action);
    u32 cancel;
