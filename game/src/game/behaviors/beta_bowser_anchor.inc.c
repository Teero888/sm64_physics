...
void bhv_beta_bowser_anchor_loop(void) {
    ...
    cur_obj_set_pos_relative(WORLD(gMarioObject), 0, 30.0f, 300.0f);

    o->hitboxRadius = WORLD(gDebugInfo)[DEBUG_PAGE_EFFECTINFO][0] + 100;
    o->hitboxHeight = WORLD(gDebugInfo)[DEBUG_PAGE_EFFECTINFO][1] + 300;
