...
void intro_peach_set_pos_and_opacity(struct Object *obj, f32 targetOpacity, f32 increment) {
    ...
    vec3f_get_dist_and_angle(WORLD(gLakituState).pos, WORLD(gLakituState).focus, &dist, &focusPitch, &focusYaw);
    vec3f_set_dist_and_angle(WORLD(gLakituState).pos, newPos, obj->oIntroPeachDistToCamera,
