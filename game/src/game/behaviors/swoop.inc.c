...
static void swoop_act_move(void) {
    ...
    if (o->oForwardVel == 0.0f) {
        ...
        // Jitter yaw a bit
        cur_obj_rotate_yaw_toward(o->oSwoopTargetYaw + (s32)(3000 * coss(4000 * WORLD(gGlobalTimer))), 1200);
        obj_roll_to_match_yaw_turn(o->oSwoopTargetYaw, 0x3000, 500);

        // Jitter roll a bit
        o->oFaceAngleRoll += (s32)(1000 * coss(20000 * WORLD(gGlobalTimer)));
    }
}
...
void bhv_swoop_update(void) {
    ...
    if (!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM)) {
        ...
        obj_check_attacks(&WORLD(sSwoopHitbox), o->oAction);
    }
}
