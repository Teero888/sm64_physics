...
void bhv_heave_ho_throw_mario_loop(void) {
    ...
    switch (o->parentObj->oHeaveHoUnk88) {
        ...
        case 2:
            ...
            WORLD(gMarioObject)->oInteractStatus |= INT_STATUS_MARIO_UNK2;
            WORLD(gMarioStates)[0].forwardVel = -45.0f;
            WORLD(gMarioStates)[0].vel[1] = 95.0f;
...
void heave_ho_act_1(void) {
    ...
    while (TRUE) {
        if (WORLD(D_8032F460)[sp1C][0] == -1) {
            o->oAction = 2;
            break;
        }

        if (o->oTimer < WORLD(D_8032F460)[sp1C][0]) {
            cur_obj_init_animation_with_accel_and_sound(2, WORLD(D_8032F460)[sp1C][1]);
            break;
        }
