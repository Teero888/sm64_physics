...
#include <ultra64.h>
// Library: not PR/os.h, which declares libultra with 32-bit addresses; the
// ROM handle functions it has are declared here (platform/ultra.c).
...
void decrease_sample_dma_ttls() {
    ...
    for (i = 0; i < WORLD(sSampleDmaListSize1); i++) {
        struct SharedDma *temp = &WORLD(sSampleDmas)[i];
        if (temp->ttl != 0) {
            temp->ttl--;
            if (temp->ttl == 0) {
                temp->reuseIndex = WORLD(sSampleDmaReuseQueueHead1);
                WORLD(sSampleDmaReuseQueue1)[WORLD(sSampleDmaReuseQueueHead1)++] = (u8) i;
            }
        }
    }

    for (i = WORLD(sSampleDmaListSize1); i < WORLD(gSampleDmaNumListItems); i++) {
        struct SharedDma *temp = &WORLD(sSampleDmas)[i];
        if (temp->ttl != 0) {
            temp->ttl--;
            if (temp->ttl == 0) {
                temp->reuseIndex = WORLD(sSampleDmaReuseQueueHead2);
                WORLD(sSampleDmaReuseQueue2)[WORLD(sSampleDmaReuseQueueHead2)++] = (u8) i;
            }
        }
    }

    WORLD(sUnused80226B40) = 0;
}
...
void *dma_sample_data(uintptr_t devAddr, u32 size, s32 arg2, u8 *dmaIndexRef, s32 medium) {
    ...
    if (arg2 != 0 || *dmaIndexRef >= WORLD(sSampleDmaListSize1)) {
        for (i = WORLD(sSampleDmaListSize1); i < WORLD(gSampleDmaNumListItems); i++) {
            dma = &WORLD(sSampleDmas)[i];
            bufferPos = devAddr - dma->source;
            if (0 <= bufferPos && (size_t) bufferPos <= dma->bufSize - size) {
                // We already have a DMA request for this memory range.
                if (dma->ttl == 0 && WORLD(sSampleDmaReuseQueueTail2) != WORLD(sSampleDmaReuseQueueHead2)) {
                    // Move the DMA out of the reuse queue, by swapping it with the
                    // tail, and then incrementing the tail.
                    if (dma->reuseIndex != WORLD(sSampleDmaReuseQueueTail2)) {
                        WORLD(sSampleDmaReuseQueue2)[dma->reuseIndex] =
                            WORLD(sSampleDmaReuseQueue2)[WORLD(sSampleDmaReuseQueueTail2)];
                        WORLD(sSampleDmas)[WORLD(sSampleDmaReuseQueue2)[WORLD(sSampleDmaReuseQueueTail2)]].reuseIndex =
                            dma->reuseIndex;
                    }
                    WORLD(sSampleDmaReuseQueueTail2)++;
                }
                dma->ttl = 60;
                *dmaIndexRef = (u8) i;
        ...
        if (WORLD(sSampleDmaReuseQueueTail2) != WORLD(sSampleDmaReuseQueueHead2) && arg2 != 0) {
            // Allocate a DMA from reuse queue 2. This queue can be empty, since
            // TTL 60 is pretty large.
            dmaIndex = WORLD(sSampleDmaReuseQueue2)[WORLD(sSampleDmaReuseQueueTail2)];
            WORLD(sSampleDmaReuseQueueTail2)++;
            dma = WORLD(sSampleDmas) + dmaIndex;
            hasDma = TRUE;
        }
    } else {
        dma = WORLD(sSampleDmas) + *dmaIndexRef;
        bufferPos = devAddr - dma->source;
        if (0 <= bufferPos && (size_t) bufferPos <= dma->bufSize - size) {
            ...
            if (dma->ttl == 0) {
                // Move the DMA out of the reuse queue, by swapping it with the
                // tail, and then incrementing the tail.
                if (dma->reuseIndex != WORLD(sSampleDmaReuseQueueTail1)) {
                    WORLD(sSampleDmaReuseQueue1)[dma->reuseIndex] =
                        WORLD(sSampleDmaReuseQueue1)[WORLD(sSampleDmaReuseQueueTail1)];
                    WORLD(sSampleDmas)[WORLD(sSampleDmaReuseQueue1)[WORLD(sSampleDmaReuseQueueTail1)]].reuseIndex =
                        dma->reuseIndex;
                }
                WORLD(sSampleDmaReuseQueueTail1)++;
            }
            dma->ttl = 2;
            return dma->buffer + (devAddr - dma->source);
        }
    ...
    if (!hasDma) {
        ...
        dmaIndex = WORLD(sSampleDmaReuseQueue1)[WORLD(sSampleDmaReuseQueueTail1)++];
        dma = WORLD(sSampleDmas) + dmaIndex;
        hasDma = TRUE;
    }
    ...
    func_sh_802f3dd0(&WORLD(gCurrAudioFrameDmaIoMesgBufs)[WORLD(gCurrAudioFrameDmaCount)++], OS_MESG_PRI_NORMAL, OS_READ,
         dmaDevAddr, dma->buffer, transfer, &WORLD(gCurrAudioFrameDmaQueue), medium, WORLD(shindouDebugPrint62));
...
void init_sample_dma_buffers(UNUSED s32 arg0) {
    ...
    WORLD(sDmaBufSize) = 0x2D0;
    WORLD(sSampleDmas) = sound_alloc_uninitialized(&WORLD(gNotesAndBuffersPool),
            WORLD(gMaxSimultaneousNotes) * 4 * sizeof(struct SharedDma) * WORLD(gAudioBufferParameters).presetUnk4);

    for (i = 0; i < WORLD(gMaxSimultaneousNotes) * 3 * WORLD(gAudioBufferParameters).presetUnk4; i++) {
        if ((WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].buffer = sound_alloc_uninitialized(&WORLD(gNotesAndBuffersPool), WORLD(sDmaBufSize))) == NULL) {
            break;
        }
        WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].bufSize = WORLD(sDmaBufSize);
        WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].source = 0;
        WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].sizeUnused = 0;
        WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].unused2 = 0;
        WORLD(sSampleDmas)[WORLD(gSampleDmaNumListItems)].ttl = 0;
        WORLD(gSampleDmaNumListItems)++;
    }

    for (i = 0; (u32) i < WORLD(gSampleDmaNumListItems); i++) {
        WORLD(sSampleDmaReuseQueue1)[i] = (u8) i;
        WORLD(sSampleDmas)[i].reuseIndex = (u8) i;
    }

    for (i = WORLD(gSampleDmaNumListItems); i < 0x100; i++) {
        WORLD(sSampleDmaReuseQueue1)[i] = 0;
    }

    WORLD(sSampleDmaReuseQueueTail1) = 0;
    WORLD(sSampleDmaReuseQueueHead1) = (u8) WORLD(gSampleDmaNumListItems);
    WORLD(sSampleDmaListSize1) = WORLD(gSampleDmaNumListItems);
    ...
    for (i = WORLD(sSampleDmaListSize1); (u32) i < WORLD(gSampleDmaNumListItems); i++) {
        WORLD(sSampleDmaReuseQueue2)[i - WORLD(sSampleDmaListSize1)] = (u8) i;
        WORLD(sSampleDmas)[i].reuseIndex = (u8)(i - WORLD(sSampleDmaListSize1));
    }

    // This probably meant to touch the range size1..size2 as well... but it
    // doesn't matter, since these values are never read anyway.
    for (i = WORLD(gSampleDmaNumListItems); i < 0x100; i++) {
        WORLD(sSampleDmaReuseQueue2)[i] = WORLD(sSampleDmaListSize1);
    }

    WORLD(sSampleDmaReuseQueueTail2) = 0;
    WORLD(sSampleDmaReuseQueueHead2) = WORLD(gSampleDmaNumListItems) - WORLD(sSampleDmaListSize1);
}
...
struct AudioBank *load_banks_immediate(s32 seqId, s32 *outDefaultBank) {
    ...
    offset = ((u16 *)WORLD(gAlBankSets))[canonicalize_index(0, seqId)];
    bank = 0xFF;
    for (i = WORLD(gAlBankSets)[offset++]; i > 0; i--) {
        bank = WORLD(gAlBankSets)[offset++];
...
s32 func_sh_802f2f38(struct AudioBankSample *sample, s32 bankId) {
    ...
    if (sample->isPatched == TRUE && sample->medium != 0) {
        ...
        if (sample->medium == 1) {
            func_sh_802f3d78((uintptr_t) sample->sampleAddr, sp24, sample->size, WORLD(gAlTbl)->unk2);
        } else {
...
void func_sh_802f3158(s32 seqId, s32 numChunks, s32 arg2, OSMesgQueue *retQueue) {
    ...
    val = ((u16 *) WORLD(gAlBankSets))[canonicalize_index(0, seqId)];
    v = WORLD(gAlBankSets)[val++];

    while (v > 0) {
        func_802f3f08(1, canonicalize_index(1, WORLD(gAlBankSets)[val++]), numChunks, arg2, retQueue);
        v--;
    }
}
...
u8 *func_sh_802f3220(u32 seqId, u32 *a1) {
    ...
    val = ((u16 *) WORLD(gAlBankSets))[canonicalize_index(0, seqId)];
    *a1 = WORLD(gAlBankSets)[val++];
    if (*a1 == 0) {
        return NULL;
    }
    return &WORLD(gAlBankSets)[val];
}
...
void func_sh_802f3288(s32 idx) {
    ...
    idx = ((u16*)WORLD(gAlBankSets))[canonicalize_index(0, idx)];
    s2 = WORLD(gAlBankSets)[idx++];
    while (s2 > 0) {
        s2--;
        bankId = canonicalize_index(1, WORLD(gAlBankSets)[idx++]);

        if (unk_pool1_lookup(1, bankId) == NULL) {
            func_sh_802f3368(bankId);
            if (WORLD(gBankLoadStatus)[bankId] != SOUND_LOAD_STATUS_5) {
                WORLD(gBankLoadStatus)[bankId] = SOUND_LOAD_STATUS_NOT_LOADED;
            }
...
BAD_RETURN(s32) func_sh_802f3368(s32 bankId) {
    struct SoundMultiPool *pool = &WORLD(gBankLoadedPool);
...
void load_sequence_internal(s32 player, s32 seqId, UNUSED s32 loadAsync) {
    ...
    seqPlayer = &WORLD(gSequencePlayers)[player];

    seqId = canonicalize_index(0, seqId);
    sequence_player_disable(seqPlayer);

    s0 = ((u16 *) WORLD(gAlBankSets))[seqId];
    count = WORLD(gAlBankSets)[s0++];
    bank = 0xff;

    while (count > 0) {
        bank = WORLD(gAlBankSets)[s0++];
...
void *func_sh_802f3598(s32 idx, s32 *medium) {
    ...
    if (ret != NULL) {
        if (WORLD(gUnkLoadStatus)[idx] != SOUND_LOAD_STATUS_5) {
            WORLD(gUnkLoadStatus)[idx] = SOUND_LOAD_STATUS_COMPLETE;
        }
...
void *func_sh_802f3688(s32 bankId) {
    ...
    bankId1 = WORLD(gCtlEntries)[bankId].bankId1;
    bankId2 = WORLD(gCtlEntries)[bankId].bankId2;
...
void *func_sh_802f3764(s32 poolIdx, s32 idx, s32 *arg2) {
    ...
    switch (poolIdx) {
        case 0:
            if (WORLD(gSeqLoadStatus)[idx] != SOUND_LOAD_STATUS_5) {
                WORLD(gSeqLoadStatus)[idx] = loadStatus;
            }
            break;

        case 1:
            if (WORLD(gBankLoadStatus)[idx] != SOUND_LOAD_STATUS_5) {
                WORLD(gBankLoadStatus)[idx] = loadStatus;
            }
            break;

        case 2:
            if (WORLD(gUnkLoadStatus)[idx] != SOUND_LOAD_STATUS_5) {
                WORLD(gUnkLoadStatus)[idx] = loadStatus;
            }
            break;
    }
...
ALSeqFile *get_audio_file_header(s32 poolIdx) {
    ...
    switch (poolIdx) {
        default:
            ...
            break;
        case 0:
            ret = WORLD(gSeqFileHeader);
            break;
        case 1:
            ret = WORLD(gAlCtlHeader);
            break;
        case 2:
            ret = WORLD(gAlTbl);
            break;
...
#define PATCH_MEM(x) x = PATCH(x, mem)
    ...
    numDrums = WORLD(gCtlEntries)[bankId].numDrums;
    numInstruments = WORLD(gCtlEntries)[bankId].numInstruments;
    ...
    WORLD(gCtlEntries)[bankId].drums = mem->drums;
    WORLD(gCtlEntries)[bankId].instruments = mem->instruments;
#undef PATCH_MEM
#undef PATCH
#undef BASE_OFFSET
}
...
again:
    if (WORLD(gAudioLoadLockSH) != 0) {
        goto again;
    }

    if (nbytes >= 0x400U) {
        func_sh_802f3dd0(&WORLD(gAudioDmaIoMesg), 1, 0, devAddr, vAddr, 0x400, &WORLD(gAudioDmaMesgQueue), medium, WORLD(shindouDebugPrint81));
        osRecvMesg(&WORLD(gAudioDmaMesgQueue), NULL, 1);
        nbytes = nbytes - 0x400;
    ...
    if (nbytes != 0) {
        func_sh_802f3dd0(&WORLD(gAudioDmaIoMesg), 1, 0, devAddr, vAddr, nbytes, &WORLD(gAudioDmaMesgQueue), medium, WORLD(shindouDebugPrint82));
        osRecvMesg(&WORLD(gAudioDmaMesgQueue), NULL, 1);
    }
}
...
s32 func_sh_802f3dd0(OSIoMesg *m, s32 pri, s32 direction, uintptr_t devAddr, void *dramAddr, s32 size, OSMesgQueue *retQueue, s32 medium, UNUSED const char *reason) {
    OSPiHandle *handle;
    if (WORLD(gAudioLoadLockSH) >= 0x11U) {
        return -1;
    }
    switch (medium) {
        case 2:
...
void *func_802f3f08(s32 poolIdx, s32 idx, s32 numChunks, s32 arg3, OSMesgQueue *retQueue) {
    ...
    switch (poolIdx) {
        case 0:
            if (WORLD(gSeqLoadStatus)[idx] == SOUND_LOAD_STATUS_IN_PROGRESS) {
                return 0;
            }
            break;
        case 1:
            if (WORLD(gBankLoadStatus)[idx] == SOUND_LOAD_STATUS_IN_PROGRESS) {
                return 0;
            }
            break;
        case 2:
            if (WORLD(gUnkLoadStatus)[idx] == SOUND_LOAD_STATUS_IN_PROGRESS) {
                return 0;
            }
            break;
...
void audio_init() {
    ...
    WORLD(gAudioLoadLockSH) = 0;

    for (i = 0; i < WORLD(gAudioHeapSize) / 8; i++) {
        ((u64 *) WORLD(gAudioHeap))[i] = 0;
    }
    ...
    WORLD(D_EU_802298D0) = 16.713f;
    WORLD(gRefreshRate) = 60;
    port_eu_init();
    ...
    for (i = 0; i < NUMAIBUFFERS; i++) {
        WORLD(gAiBufferLengths)[i] = 0xa0;
    }

    WORLD(gAudioFrameCount) = 0;
    WORLD(gAudioTaskIndex) = 0;
    WORLD(gCurrAiBufferIndex) = 0;
    WORLD(gSoundMode) = 0;
    WORLD(gAudioTask) = NULL;
    WORLD(gAudioTasks)[0].task.t.data_size = 0;
    WORLD(gAudioTasks)[1].task.t.data_size = 0;
    osCreateMesgQueue(&WORLD(gAudioDmaMesgQueue), &WORLD(gAudioDmaMesg), 1);
    osCreateMesgQueue(&WORLD(gCurrAudioFrameDmaQueue), WORLD(gCurrAudioFrameDmaMesgBufs),
                      ARRAY_COUNT(WORLD(gCurrAudioFrameDmaMesgBufs)));
    osCreateMesgQueue(&WORLD(gUnkQueue1), WORLD(gUnkMesgBufs1), 0x10);
    osCreateMesgQueue(&WORLD(gUnkQueue2), WORLD(gUnkMesgBufs2), 0x10);
    WORLD(gCurrAudioFrameDmaCount) = 0;
    WORLD(gSampleDmaNumListItems) = 0;

    sound_init_main_pools(WORLD(gAudioInitPoolSize));

    for (i = 0; i < NUMAIBUFFERS; i++) {
        WORLD(gAiBuffers)[i] = sound_alloc_uninitialized(&WORLD(gAudioInitPool), AIBUFFER_LEN);
    ...
    WORLD(gAudioResetPresetIdToLoad) = 0;
    WORLD(gAudioResetStatus) = 1;
    ...
    WORLD(gSeqFileHeader) = (ALSeqFile *) WORLD(gShindouSequencesHeader);
    WORLD(gAlCtlHeader) = (ALSeqFile *) WORLD(gShindouSoundBanksHeader);
    WORLD(gAlTbl) = (ALSeqFile *) WORLD(gShindouSampleBanksHeader);
    WORLD(gAlBankSets) = WORLD(gBankSetsData);
    WORLD(gSequenceCount) = (s16) WORLD(gSeqFileHeader)->seqCount;
    patch_seq_file(WORLD(gSeqFileHeader), gMusicData, WORLD(D_SH_80315EF4));
    patch_seq_file(WORLD(gAlCtlHeader), gSoundDataADSR, WORLD(D_SH_80315EF8));
    patch_seq_file(WORLD(gAlTbl), gSoundDataRaw, WORLD(D_SH_80315EFC));
    seqCount = WORLD(gAlCtlHeader)->seqCount;
    WORLD(gCtlEntries) = sound_alloc_uninitialized(&WORLD(gAudioInitPool), seqCount * sizeof(struct CtlEntry));
    for (i = 0; i < seqCount; i++) {
        WORLD(gCtlEntries)[i].bankId1 = (u8) ((WORLD(gAlCtlHeader)->seqArray[i].ctl.as_s16.bankAndFf >> 8) & 0xff);
        WORLD(gCtlEntries)[i].bankId2 = (u8) (WORLD(gAlCtlHeader)->seqArray[i].ctl.as_s16.bankAndFf & 0xff);
        WORLD(gCtlEntries)[i].numInstruments = (u8) ((WORLD(gAlCtlHeader)->seqArray[i].ctl.as_s16.numInstrumentsAndDrums >> 8) & 0xff);
        WORLD(gCtlEntries)[i].numDrums = (u8) (WORLD(gAlCtlHeader)->seqArray[i].ctl.as_s16.numInstrumentsAndDrums & 0xff);
    }
    data = sound_alloc_uninitialized(&WORLD(gAudioInitPool), WORLD(D_SH_80315EF0));
    if (data == NULL) {
        WORLD(D_SH_80315EF0) = 0;
    }
    sound_alloc_pool_init(&WORLD(gUnkPool1).pool, data, WORLD(D_SH_80315EF0));
...
s32 func_sh_802f47c8(s32 bankId, u8 idx, s8 *io) {
    ...
    temp = &WORLD(D_SH_80343D00).arr[WORLD(D_SH_80343D00).someIndex];
    if (temp->state == 3) {
        temp->state = 0;
    }
    temp->sample = *sample;
    ...
    temp->bankId = bankId;
    temp->idx = idx;
    WORLD(D_SH_80343D00).someIndex ^= 1;
    return 0;
}
...
void func_sh_802f4a4c(s32 audioResetStatus) {
    ...
    for (i = 0; i < 2; i++) {
        entry = &WORLD(D_SH_80343D00).arr[i];
        switch (entry->state) {
            case 2:
...
void func_sh_802f4bd8(struct PendingDmaSample *arg0, s32 len) { // len must be signed
    ...
    func_sh_802f3dd0(&arg0->ioMesg, 0, 0, arg0->devAddr, arg0->vAddr, len, &arg0->queue, arg0->medium, WORLD(shindouDebugPrint102));
}
...
struct PendingDmaAudioBank *func_sh_802f4cb4(uintptr_t devAddr, void *vAddr, s32 size, s32 medium, s32 numChunks, OSMesgQueue *retQueue, s32 encodedInfo) {
    ...
    for (i = 0; i < ARRAY_COUNT(WORLD(sPendingDmaAudioBanks)); i++) {
        if (WORLD(sPendingDmaAudioBanks)[i].inProgress == 0) {
            item = &WORLD(sPendingDmaAudioBanks)[i];
            break;
        }
    }
    if (i == ARRAY_COUNT(WORLD(sPendingDmaAudioBanks))) {
        return NULL;
    }
...
void func_sh_802f4dcc(s32 audioResetStatus) {
    ...
    if (WORLD(gAudioLoadLockSH) != 1) {
        for (i = 0; i < ARRAY_COUNT(WORLD(sPendingDmaAudioBanks)); i++) {
            if (WORLD(sPendingDmaAudioBanks)[i].inProgress == 1) {
                func_sh_802f4e50(&WORLD(sPendingDmaAudioBanks)[i], audioResetStatus);
            }
        }
    }
}
...
#pragma GCC diagnostic pop
        ...
        switch ((u8) (temp >> 0x10)) {
            case 0:
                if (WORLD(gSeqLoadStatus)[bankId] != SOUND_LOAD_STATUS_5) {
                    WORLD(gSeqLoadStatus)[bankId] = (u8) (temp & 0xFF);
                }
                break;
            case 2:
                if (WORLD(gUnkLoadStatus)[bankId] != SOUND_LOAD_STATUS_5) {
                    WORLD(gUnkLoadStatus)[bankId] = (u8) (temp & 0xFF);
                }
                break;
            case 1:
                if (WORLD(gBankLoadStatus)[bankId] != SOUND_LOAD_STATUS_5) {
                    WORLD(gBankLoadStatus)[bankId] = (u8) (temp & 0xFF);
                }
                bankId1 = WORLD(gCtlEntries)[bankId].bankId1;
                bankId2 = WORLD(gCtlEntries)[bankId].bankId2;
...
void func_sh_802f50ec(struct PendingDmaAudioBank *arg0, size_t len) {
    ...
    func_sh_802f3dd0(&arg0->ioMesg, 0, 0, arg0->devAddr, arg0->vAddr, len, &arg0->dmaRetQueue, arg0->medium, WORLD(shindouDebugPrint110));
}
    ...
    if ((uintptr_t) sound->sample <= 0x80000000) {
        ...
        if (sample->size != 0 && sample->isPatched != TRUE) {
            ...
            if (sample->bit1 && sample->medium != 0) {
                WORLD(D_SH_8034EA88)[WORLD(D_SH_8034F688)++] = sample;
            }
        }
    }
#undef PATCH
...
BAD_RETURN(s32) func_sh_802f5310(s32 bankId, struct AudioBank *mem, struct PatchStruct *patchInfo, s32 arg3) {
    ...
    if (WORLD(D_SH_8034F68C) != 0) {
        ...
        WORLD(D_SH_80343CF0) = 0;
    }
    WORLD(D_SH_8034F688) = 0;
    patch_audio_bank(bankId, mem, patchInfo);

    count = 0;
    for (i = 0; i < WORLD(D_SH_8034F688); i++) {
        count += ALIGN16(WORLD(D_SH_8034EA88)[i]->size);
    }

    for (i = 0; i < WORLD(D_SH_8034F688); i++) {
        if (WORLD(D_SH_8034F68C) != 0x78) {
            temp_s0 = WORLD(D_SH_8034EA88)[i];
            switch (arg3) {
                case 0:
            ...
            switch ((uintptr_t) addr) {
                ...
                switch (arg3) {
                    case 0:
                        if (temp_s0->medium == 1) {
                            func_sh_802f3d78((uintptr_t) temp_s0->sampleAddr, addr, temp_s0->size, WORLD(gAlTbl)->unk2);
                    ...
                    case 1:
                        WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C)].sample = temp_s0;
                        WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C)].ramAddr = addr;
                        WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C)].encodedInfo = (WORLD(D_SH_8034F68C) << 24) | 0xffffff;
                        WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C)].isFree = FALSE;
                        WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C)].endAndMediumIdentification = temp_s0->sampleAddr + temp_s0->size + temp_s0->medium;
                        WORLD(D_SH_8034F68C)++;
                        break;
                }
            }
    ...
    WORLD(D_SH_8034F688) = 0;
    if (WORLD(D_SH_8034F68C) != 0 && sp4C == 0) {
        temp_s0 = WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].sample;
        temp = (temp_s0->size >> 12);
        temp += 1;
        count = (uintptr_t) temp_s0->sampleAddr;
        func_sh_802f4cb4(
            count,
            WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].ramAddr,
            temp_s0->size,
            temp_s0->medium,
            temp,
            &WORLD(gUnkQueue2),
            WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].encodedInfo);
    }
}
...
s32 func_sh_802f573c(s32 audioResetStatus) {
    ...
    if (WORLD(D_SH_8034F68C) > 0) {
        ...
        if (WORLD(D_SH_8034EC88)[idx].isFree == FALSE) {
            ...
            if (added == WORLD(D_SH_8034EC88)[idx].endAndMediumIdentification) {
                sample->sampleAddr = WORLD(D_SH_8034EC88)[idx].ramAddr;
                sample->medium = 0;
            }
            WORLD(D_SH_8034EC88)[idx].isFree = TRUE;
        }

next:
        if (WORLD(D_SH_8034F68C) > 0) {
            if (WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].isFree == TRUE) {
                WORLD(D_SH_8034F68C)--;
                goto next;
            }
            sample = WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].sample;
            sampleAddr = sample->sampleAddr;
            size = sample->size;
            unk = size >> 0xC;
            unk += 1;
            added = ((sampleAddr + size) + sample->medium);
            if (added != WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].endAndMediumIdentification) {
                WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].isFree = TRUE;
                WORLD(D_SH_8034F68C)--;
                goto next;
            }
            size = sample->size;
            func_sh_802f4cb4((uintptr_t) sampleAddr, WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].ramAddr, size, sample->medium,
                             unk, &WORLD(gUnkQueue2), WORLD(D_SH_8034EC88)[WORLD(D_SH_8034F68C) - 1].encodedInfo);
        }
    }
    return 1;
}
...
#include "pointers/game/src/audio/load_sh.c.inc.c"
