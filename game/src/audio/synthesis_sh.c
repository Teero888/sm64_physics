...
u64 *synthesis_save_reverb_ring_buffer(u64 *cmd, u16 addr, u16 destOffset, s32 len, s32 reverbIndex) {
    ...
    aSaveBuffer(cmd++, addr + DEFAULT_LEN_1CH,
                VIRTUAL_TO_PHYSICAL2(&WORLD(gSynthesisReverbs)[reverbIndex].ringBuffer.right[destOffset]), len);
    return cmd;
}
...
void func_sh_802ed644(s32 updateIndexStart, s32 noteIndex) {
    ...
    for (i = updateIndexStart + 1; i < WORLD(gAudioBufferParameters).updatesPerFrame; i++) {
        if (!WORLD(gNoteSubsEu)[WORLD(gMaxSimultaneousNotes) * i + noteIndex].needsInit) {
            WORLD(gNoteSubsEu)[WORLD(gMaxSimultaneousNotes) * i + noteIndex].enabled = FALSE;
        } else {
            break;
        }
    }
    ...
    for (i = WORLD(gAudioBufferParameters).updatesPerFrame; i > 0; i--) {
        if (i == 1) {
            ...
            if (bufLen / i >= WORLD(gAudioBufferParameters).samplesPerUpdateMax) {
                chunkLen = WORLD(gAudioBufferParameters).samplesPerUpdateMax;
            } else if (bufLen / i <= WORLD(gAudioBufferParameters).samplesPerUpdateMin) {
                chunkLen = WORLD(gAudioBufferParameters).samplesPerUpdateMin;
            } else {
                chunkLen = WORLD(gAudioBufferParameters).samplesPerUpdate;
            }
        }
...
u64 *synthesis_resample_and_mix_reverb(u64 *cmd, s32 bufLen, s16 reverbIndex, s16 updateIndex) {
    ...
    if (WORLD(gSynthesisReverbs)[reverbIndex].downsampleRate == 1) {
        ...
        aMix(cmd++, 0x8000 + WORLD(gSynthesisReverbs)[reverbIndex].reverbGain, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_WET_LEFT_CH, DEFAULT_LEN_2CH);
    } else {
    ...
    if (WORLD(gSynthesisReverbs)[reverbIndex].panRight != 0 || WORLD(gSynthesisReverbs)[reverbIndex].panLeft != 0) {
        ...
        aMix(cmd++, WORLD(gSynthesisReverbs)[reverbIndex].panRight, DMEM_ADDR_WET_RIGHT_CH, DMEM_ADDR_WET_LEFT_CH, DEFAULT_LEN_1CH);
        aMix(cmd++, WORLD(gSynthesisReverbs)[reverbIndex].panLeft, DMEM_ADDR_RESAMPLED, DMEM_ADDR_WET_RIGHT_CH, DEFAULT_LEN_1CH);
    }
    return cmd;
}
...
u64 *synthesis_load_reverb_samples(u64 *cmd, s16 reverbIndex, s16 updateIndex) {
    ...
    reverb = &WORLD(gSynthesisReverbs)[reverbIndex];
    item = &reverb->items[reverb->curFrame][updateIndex];
    ...
    switch (WORLD(gSynthesisReverbs)[reverbIndex].downsampleRate) {
        ...
        default:
            ...
            aSaveBuffer(cmd++, DMEM_ADDR_WET_LEFT_CH,
                    VIRTUAL_TO_PHYSICAL2(WORLD(gSynthesisReverbs)[reverbIndex].items[WORLD(gSynthesisReverbs)[reverbIndex].curFrame][updateIndex].toDownsampleLeft), DEFAULT_LEN_2CH);
            break;
    }
...
u64 *synthesis_do_one_audio_update(s16 *aiBuf, s32 bufLen, u64 *cmd, s32 updateIndex) {
    ...
    for (j = 0; j < WORLD(gNumSynthesisReverbs); j++) {
        ...
        if (WORLD(gSynthesisReverbs)[j].useReverb != 0) {
            if (WORLD(gSynthesisReverbs)[j].unk100 != NULL) {
                aFilter(cmd++, 0x02, bufLen * 2, WORLD(gSynthesisReverbs)[j].unk100);
                aFilter(cmd++, WORLD(gSynthesisReverbs)[j].resampleFlags, DMEM_ADDR_WET_LEFT_CH, WORLD(gSynthesisReverbs)[j].unk108);
            }
            if (WORLD(gSynthesisReverbs)[j].unk104 != NULL) {
                aFilter(cmd++, 0x02, bufLen * 2, WORLD(gSynthesisReverbs)[j].unk104);
                aFilter(cmd++, WORLD(gSynthesisReverbs)[j].resampleFlags, DMEM_ADDR_WET_RIGHT_CH, WORLD(gSynthesisReverbs)[j].unk10C);
            }
            cmd = synthesis_save_reverb_samples(cmd, j, updateIndex);
            if (WORLD(gSynthesisReverbs)[j].unk5 != -1) {
                if (WORLD(gSynthesisReverbs)[WORLD(gSynthesisReverbs)[j].unk5].downsampleRate == 1) {
                    cmd = synthesis_load_reverb_samples(cmd, WORLD(gSynthesisReverbs)[j].unk5, updateIndex);
                    aMix(cmd++, WORLD(gSynthesisReverbs)[j].unk08, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_RESAMPLED, DEFAULT_LEN_2CH);
                    cmd = func_sh_802EDF24(cmd++, WORLD(gSynthesisReverbs)[j].unk5, updateIndex);
                }
            }
        }
    }
    for (; i < notePos; i++) {
        struct NoteSubEu *noteSubEu2 = &WORLD(gNoteSubsEu)[updateIndex * WORLD(gMaxSimultaneousNotes) + noteIndices[i]];
        cmd = synthesis_process_note(noteIndices[i],
                                     noteSubEu2,
                                     &WORLD(gNotes)[noteIndices[i]].synthesisState,
                                     aiBuf, bufLen, cmd, updateIndex);
            ...
            if (audioBookSample->codec == CODEC_ADPCM) {
                if (curLoadedBook != (*audioBookSample->book).book) {
                    ...
                    switch (noteSubEu->bookOffset) {
                        case 1:
                            curLoadedBook = WORLD(euUnknownData_80301950) + 1;
                            break;
                        case 2:
                            curLoadedBook = WORLD(euUnknownData_80301950) + 2;
                            break;
                        case 3:
                        default:
...
#include "pointers/game/src/audio/synthesis_sh.c.inc.c"
