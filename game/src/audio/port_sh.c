...
struct SPTask *create_next_audio_frame_task(void) {
    ...
    if (WORLD(gAudioFrameCount) % WORLD(gAudioBufferParameters).presetUnk4 != 0) {
        if ((WORLD(gAudioFrameCount) % WORLD(gAudioBufferParameters).presetUnk4) + 1 == WORLD(gAudioBufferParameters).presetUnk4) {
            return WORLD(D_SH_80314FCC);
        }
        return NULL;
    }
    osSendMesg(WORLD(D_SH_80350F38), (OSMesg) WORLD(gAudioFrameCount), OS_MESG_NOBLOCK);
    ...
    if (WORLD(gAudioLoadLockSH) < 0x10U) {
    ...
    if (osRecvMesg(WORLD(D_SH_80350F88), (OSMesg *) &sp38, OS_MESG_NOBLOCK) != -1) {
        ...
        WORLD(gAudioResetPresetIdToLoad) = (u8) sp38;
    }

    if (WORLD(gAudioResetStatus) != 0) {
        if (audio_shut_down_and_reset_step() == 0) {
            if (WORLD(gAudioResetStatus) == 0) {
                osSendMesg(WORLD(D_SH_80350FA8), (OSMesg) (s32) WORLD(gAudioResetPresetIdToLoad), OS_MESG_NOBLOCK);
            }
            WORLD(D_SH_80314FCC) = 0;
            return NULL;
        }
    }
    ...
    WORLD(gAiBufferLengths)[index] = (s16) ((((WORLD(gAudioBufferParameters).samplesPerFrameTarget - samplesRemainingInAI) +
    ...
    WORLD(gAudioRandom) = (u32) (osGetCount() * (WORLD(gAudioRandom) + WORLD(gAudioFrameCount)));
    WORLD(gAudioRandom) = (u32) (WORLD(gAiBuffers)[index][WORLD(gAudioFrameCount) & 0xFF] + WORLD(gAudioRandom));
    ...
    task->ucode_boot_size = (u8 *) WORLD(rspF3DStart) - (u8 *) WORLD(rspF3DBootStart);
    ...
    if (WORLD(D_SH_80314FC8) < writtenCmdsCopy) {
        WORLD(D_SH_80314FC8) = writtenCmdsCopy;
    }

    if (WORLD(gAudioBufferParameters).presetUnk4 == 1) {
        return WORLD(gAudioTask);
    } else {
        WORLD(D_SH_80314FCC) = WORLD(gAudioTask);
        return NULL;
    }
}
...
void eu_process_audio_cmd(struct EuAudioCmd *cmd) {
    ...
    switch (cmd->u.s.op) {
        ...
        case 0xf4:
            func_sh_802f30f4(cmd->u.s.arg1, cmd->u.s.arg2, cmd->u.s.arg3, &WORLD(gUnkQueue1));
            break;

        case 0xf5:
            func_sh_802f3158(cmd->u.s.arg1, cmd->u.s.arg2, cmd->u.s.arg3, &WORLD(gUnkQueue1));
            break;
...
void seq_player_fade_to_zero_volume(s32 arg0, s32 fadeOutTime) {
    ...
    player = &WORLD(gSequencePlayers)[arg0];
    player->state = 2;
...
void func_8031D690(s32 playerIndex, s32 fadeInTime) {
    ...
    if (fadeInTime != 0) {
        player = &WORLD(gSequencePlayers)[playerIndex];
        player->state = 1;
...
void port_eu_init_queues(void) {
    WORLD(D_SH_80350F18) = 0;
    WORLD(D_SH_80350F19) = 0;
    WORLD(D_SH_80350F38) = &WORLD(D_SH_80350F20);
    WORLD(D_SH_80350F68) = &WORLD(D_SH_80350F50);
    WORLD(D_SH_80350F88) = &WORLD(D_SH_80350F70);
    WORLD(D_SH_80350FA8) = &WORLD(D_SH_80350F90);
    osCreateMesgQueue(WORLD(D_SH_80350F38), WORLD(D_SH_80350F1C), 1);
    osCreateMesgQueue(WORLD(D_SH_80350F68), WORLD(D_SH_80350F40), 4);
    osCreateMesgQueue(WORLD(D_SH_80350F88), WORLD(D_SH_80350F6C), 1);
    osCreateMesgQueue(WORLD(D_SH_80350FA8), WORLD(D_SH_80350F8C), 1);
}

extern struct EuAudioCmd sAudioCmd[0x100];
void func_802ad6f0(s32 arg0, s32 *arg1) {
    struct EuAudioCmd *cmd = &WORLD(sAudioCmd)[WORLD(D_SH_80350F18) & 0xff];
    cmd->u.first = arg0;
    cmd->u2.as_u32 = *arg1;
    WORLD(D_SH_80350F18)++;
    if (WORLD(D_SH_80350F18) == WORLD(D_SH_80350F19)) {
        WORLD(D_SH_80350F18)--;
    }
}
...
void func_sh_802F64C8(void) {
    ...
    if (((WORLD(D_SH_80350F18) - WORLD(D_SH_80350F19) + 0x100) & 0xff) > WORLD(D_SH_8031503C)) {
        WORLD(D_SH_8031503C) = (WORLD(D_SH_80350F18) - WORLD(D_SH_80350F19) + 0x100) & 0xff;
    }
    mesg = ((WORLD(D_SH_80350F19) & 0xff) << 8) | (WORLD(D_SH_80350F18) & 0xff);
    osSendMesg(WORLD(D_SH_80350F68), (OSMesg)mesg, OS_MESG_NOBLOCK);
    WORLD(D_SH_80350F19) = WORLD(D_SH_80350F18);
}
...
void func_802ad7ec(u32 arg0) {
    ...
    if (WORLD(D_SH_8031509C) == 0) {
        WORLD(D_SH_80315098) = (arg0 >> 8) & 0xff;
    }

    end = arg0 & 0xff;

    for (;;) {
        if (WORLD(D_SH_80315098) == end) {
            WORLD(D_SH_8031509C) = 0;
            break;
        }
        cmd = &WORLD(sAudioCmd)[WORLD(D_SH_80315098) & 0xff];
        WORLD(D_SH_80315098)++;
        if (cmd->u.s.op == 0xf8) {
            WORLD(D_SH_8031509C) = 1;
            break;
        }
...
void func_sh_802f6958(OSMesg mesg) {
    ...
    do {
        a = -1;
    } while (osRecvMesg(WORLD(D_SH_80350FA8), &recvMesg, OS_MESG_NOBLOCK) != a);
    func_sh_802f6540();
    osSendMesg(WORLD(D_SH_80350F88), mesg, OS_MESG_NOBLOCK);
}

void func_sh_802f69cc(void) {
    WORLD(gAudioLoadLockSH) = 1;
    func_sh_802f6958(0);
...
s8 func_sh_802f6a6c(s32 playerIndex, s32 index) {
    return WORLD(gSequencePlayers)[playerIndex].seqVariationEu[index];
}
...
#include "pointers/game/src/audio/port_sh.c.inc.c"
