...
"""Movies made up for a version without a TAS to check it with.
  ...
  make_movie.py idle OUT.m64 POLLS [VI_RATE]
  make_movie.py random OUT.m64 POLLS SEED [VI_RATE]
...
def main():
    ...
    with open(path, "wb") as f:
        f.write(bytes(header(polls, vi_rate)) + bytes(body))
